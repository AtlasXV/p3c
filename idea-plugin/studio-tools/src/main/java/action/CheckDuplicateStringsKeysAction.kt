 ...
 * 2020年08月17日11:31:03
 */
class CheckDuplicateStringsKeysAction : AnAction {
    constructor() : super() {}
    constructor(
    ...
    private fun alertSameStrings(
        ...
        var targetFile = reportsDir?.findChild("duplicate-key-strings.txt")
        if (targetFile?.exists() == true) {
            targetFile.delete(null)
        }
        targetFile = reportsDir?.createChildData(null, "duplicate-key-strings.txt")
        ...
        Messages.showMessageDialog(
            currentProject,
            "A report is generated: rootProjectDir/studioTools/reports/duplicate-key-strings.txt",
            dlgTitle,
            Messages.getInformationIcon()
        )
    }
